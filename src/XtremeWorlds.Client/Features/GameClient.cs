...
public class GameClient : Game
{
    ...
    private static long _fps;

    public static void SetFps(long fps)
    {
        Interlocked.Exchange(ref _fps, fps);
    }

    public static long GetFps()
    {
        return Interlocked.Read(ref _fps);
    }
    ...
    public static GfxInfo? GetGfxInfo(string key)
    {
        ...
        if (!GfxInfoCache.TryGetValue(key, out var result))
        {
    ...
    public GameClient()
    {
        ...
        Graphics.IsFullScreen = SettingsManager.Instance.Fullscreen;
        Graphics.PreferredBackBufferWidth = GameState.ResolutionWidth;
        Graphics.PreferredBackBufferHeight = GameState.ResolutionHeight;
        Graphics.SynchronizeWithVerticalRetrace = SettingsManager.Instance.Vsync;
    ...
    public static ITextureService TextureService { get; private set; }
    ...
    protected override void Initialize()
    {
        ...
        TextureService = new TextureService(GraphicsDevice);
        ...
        Components.Add(new UIInputComponent(this));
        Components.Add(new UIRendererComponent(this));
    ...
    protected override void BeginRun()
    {
        ...
        {
            Serilog.Log.Error(ex, "Failed to start network");
        }
    }

    private static void LoadFonts()
    {
        var fontValues = Enum.GetValues<Font>();
    ...
    private static void HandleTextInput()
    {
        ...
        foreach (var key in CurrentKeyboardState.GetPressedKeys())
        {
            ...
            if (key is Keys.Tab or Keys.LeftShift or Keys.RightShift or Keys.LeftControl or Keys.RightControl or Keys.LeftAlt or Keys.RightAlt)
            {
                continue;
            }
    ...
    public static void DrawNpc(int mapNpcNum)
    {
        ...
        // Check if Npc exists
        if (Data.MyMapNpc[mapNpcNum].Num < 0 ||
            Data.MyMapNpc[mapNpcNum].Num > Constant.MaxNpcs)
            return;

        x = (int) Math.Floor((double) Data.MyMapNpc[mapNpcNum].X / 32);
        y = (int) Math.Floor((double) Data.MyMapNpc[mapNpcNum].Y / 32);
        ...
        Database.StreamNpc(Data.MyMapNpc[mapNpcNum].Num);

        // Get the sprite of the Npc
        sprite = Data.Npc[Data.MyMapNpc[mapNpcNum].Num].Sprite;
        ...
        if (Data.MyMapNpc[mapNpcNum].AttackTimer + attackSpeed / 2d > General.GetTickCount() &&
        ...
        {
            anim = (byte) Data.MyMapNpc[mapNpcNum].Steps;
        }
        ...
        switch (Data.MyMapNpc[mapNpcNum].Dir)
        {
        ...
        x = (int) Math.Round(Data.MyMapNpc[mapNpcNum].X -
                             (GetGfxInfo(Path.Combine(DataPath.Characters, sprite.ToString())).Width /
                              4d -
                              32d) / 2d);
        ...
        if (GetGfxInfo(Path.Combine(DataPath.Characters, sprite.ToString())).Height / 4d > 32d)
        {
            ...
            y = (int) Math.Round(Data.MyMapNpc[mapNpcNum].Y -
    ...
    public static void DrawBars()
    {
        ...
        {
            npcNum = Data.MyMapNpc[(int) i].Num;
            // exists?
            if (npcNum >= 0L && npcNum <= Constant.MaxNpcs)
            {
                    ...
                    Data.MyMapNpc[(int) i].Vital[(int) Vital.Health] < Data.Npc[(int) npcNum].Hp)
                {
                    ...
                    RenderTexture(ref argpath1, GameLogic.ConvertMapX((int) tmpX), GameLogic.ConvertMapY((int) tmpY),
                        (int) left, (int) top, GameState.BarWidthNpcHp[(int) i], (int) height,
                        GameState.BarWidthNpcHp[(int) i], (int) height);
                }
            }
        }
            ...
            if (GetPlayerMap((int) i) == GetPlayerMap((int) i))
            {
                    ...
                    GetPlayerVital((int) i, Vital.Health) < GetPlayerMaxVital((int) i, Vital.Health))
                {
                    ...
                    RenderTexture(ref argpath3, GameLogic.ConvertMapX((int) tmpX), GameLogic.ConvertMapY((int) tmpY),
                        (int) left, (int) top, GameState.BarWidthPlayerHp[(int) i], (int) height,
                        GameState.BarWidthPlayerHp[(int) i], (int) height);
                }
                    ...
                    GetPlayerVital((int) i, Vital.Stamina) < GetPlayerMaxVital((int) i, Vital.Stamina))
                {
                    ...
                    RenderTexture(ref argpath5, GameLogic.ConvertMapX((int) tmpX), GameLogic.ConvertMapY((int) tmpY),
                        (int) left, (int) top, GameState.BarWidthPlayerSp[(int) i], (int) height,
                        GameState.BarWidthPlayerSp[(int) i], (int) height);
                }

                if (GameState.SkillBuffer >= 0)
                {
                    if (Data.Player[(int) i].Skill[GameState.SkillBuffer].Num >= 0)
                    {
                        if (Data.Skill[Data.Player[(int) i].Skill[GameState.SkillBuffer].Num]
                                .CastTime >
                            0)
                        {
                            // lock to player
                            ...
                            if (width > 0L)
                                barWidth = (long) Math.Round((General.GetTickCount() - GameState.SkillBufferTimer) /
                                    (double) (Data
                                        .Skill[Data.Player[(int) i].Skill[GameState.SkillBuffer].Num]
                                        .CastTime * 1000) * width);
    ...
    public static void DrawChatBubble(long index)
    {
...

        {
            ...
            RenderTexture(ref argpath3, (int) (x2 - 9L), y, 0, 19, 9, 6, 9, 6);

            // bottom right
            var argpath4 = Path.Combine(DataPath.Gui, 33.ToString());
            RenderTexture(ref argpath4, (int) (x2 + maxWidth), y, 119, 19, 9, 6, 9, 6);

            // bottom - left half
            var argpath5 = Path.Combine(DataPath.Gui, 33.ToString());
            RenderTexture(ref argpath5, (int) x2, y, 9, 19, (int) (maxWidth / 2L - 5L), 6, 6, 6);

            // bottom - right half
            var argpath6 = Path.Combine(DataPath.Gui, 33.ToString());
            RenderTexture(ref argpath6, (int) (x2 + maxWidth / 2L + 6L), y, 9, 19, (int) (maxWidth / 2L - 5L), 6,
                9,
                6);
            ...
            RenderTexture(ref argpath10, (int) (x - 5L), y, 58, 19, 11, 11, 11, 11);
    ...
    public static void DrawPlayer(int index)
    {
        ...
        var isUp = dirVal is Direction.Up or Direction.UpLeft or Direction.UpRight;
        var isDown = dirVal is Direction.Down or Direction.DownLeft or Direction.DownRight;
        ...
        try
        {
            ...
            switch (Data.MapEvents[id].GraphicType)
            {
                ...
                case 1:
                {
                    ...
                    width = (int) Math.Round(gfxInfo.Width / 4d);
