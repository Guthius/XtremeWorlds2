...
public class Sound
{
    ...
    {
        var path = System.IO.Path.Combine(DataPath.Music, fileName);
    ...
    public static void InitializeBass()
    {
        ...
        var soundFontPath = "GeneralUser.sf2";
    ...
    public static void PlayMidi(string filePath)
    {
        ...
        var fonts = new MidiFont[] { font };

        // Set the fonts for the MIDi stream and check if it succeeded
        var fontCount = BassMidi.StreamSetFonts(MusicStream, fonts, fonts.Length);
    ...
    public static void PlaySound(string fileName, int x, int y, bool looped = false)
    {
        ...
        try
        {
            ...
            if (SoundStream != 0)
            {
                var calculatedVolume = CalculateSoundVolume(ref x, ref y);
    ...
    public static void FadeIn()
    {
        if (MusicStream != 0)
        {
            Bass.ChannelGetAttribute(MusicStream, ChannelAttribute.Volume, out var currentVolume);
    ...
    public static void PlayWeatherSound(string fileName, bool looped = false)
    {
        ...
        var soundPath = DataPath.Sounds + fileName;
