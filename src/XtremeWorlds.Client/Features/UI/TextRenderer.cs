...
public static class TextRenderer
{
    ...
    static TextRenderer()
    {
        _fontSystem.AddFont(File.ReadAllBytes(Path.Combine("Content", "Fonts", "Coolvetica Rg.otf")));
    }

    public static SpriteFontBase GetFont()
    {
        return _fontSystem.GetFont(16);
    }
    ...
    public static void RenderText(string text, int x, int y, Color frontColor, Color backColor, Font font = Font.Georgia)
    {
        ...
        var f = _fontSystem.GetFont(18);
        
        GameClient.SpriteBatch.DrawString(f, text, new Vector2(x + 1, y + 1), backColor);
        GameClient.SpriteBatch.DrawString(f, text, new Vector2(x, y), frontColor);
    }
