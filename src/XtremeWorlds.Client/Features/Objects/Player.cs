...
public class Player
{
    ...
    public static void ClearPlayer(int index)
    {
        ...
        for (var y = 0; y < Data.Player[index].Equipment.Length; y++)
        ...
        for (var x = 0; x < Constant.MaxInv; x++)
        {
        ...
        for (var x = 0; x < Constant.MaxPlayerSkills; x++)
        {
            ...
            if (General.GetTickCount() > Data.Player[GameState.MyIndex].EventTimer)
            {
                for (int i = 0, loopTo = GameState.CurrentEvents; i < loopTo; i++)
                {
                    ...
                    if (Data.MapEvents[i].Visible == true)
                    {
                        ...
                        var eventX = Data.MapEvents[i].X;
                        var eventY = Data.MapEvents[i].Y;
    ...
    public static void Packet_PlayerData(ReadOnlyMemory<byte> data)
    {
        ...
        var resourceSkillCount = Enum.GetValues(typeof(ResourceSkill)).Length;
        ...
        if (i == GameState.MyIndex)
        {
            ...
            // set form
            {
                ...
                // withBlock.GetChild("lblName").Text = "Name";
                // withBlock.GetChild("lblJob").Text = "Job";
                // withBlock.GetChild("lblLevel").Text = "Level";
                // withBlock.GetChild("lblGuild").Text = "Guild";
                withBlock.GetChild("lblName2").Text = GetPlayerName(GameState.MyIndex);
                withBlock.GetChild("lblJob2").Text = Data.Job[GetPlayerJob(GameState.MyIndex)].Name;
                withBlock.GetChild("lblLevel2").Text = GetPlayerLevel(GameState.MyIndex).ToString();
                withBlock.GetChild("lblGuild2").Text = "None";
                WinCharacter.Update();

                var hasPointsToSpend = GetPlayerPoints(GameState.MyIndex) > 0;
                
                // stats
                for (x = 0; x < statCount; x++)
                {
                    var statLabel = withBlock.GetChild<Label>("lblStat_" + (x + 1));
                    var statButton = withBlock.GetChild<Button>("btnStat_" + (x + 1));
                    
                    statLabel.Text = GetPlayerStat(GameState.MyIndex, (Stat) x).ToString();
                    statButton.Enabled = hasPointsToSpend;
                }

                // points
                withBlock.GetChild("lblPoints").Text = GetPlayerPoints(GameState.MyIndex).ToString();
            }
