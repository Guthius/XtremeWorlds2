...
public class Map
{
    ...
    public static void DrawWeather()
    {
        ...
        for (i = 0; i < Constant.MaxWeatherParticles; i++)
        {
            if (Conversions.ToBoolean(GameState.WeatherParticle[i].InUse))
            {
                ...
                var argpath = System.IO.Path.Combine(DataPath.Misc, "Weather");
    ...
    {
        var fogNum = GameState.CurrentFog;
        ...
        var sX = 0;
        var sY = 0;
        var sW = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Fogs, fogNum.ToString())).Width; // Using the full width of the fog texture
        var sH = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Fogs, fogNum.ToString())).Height; // Using the full height of the fog texture

        // These should match the scale calculations for full coverage plus extra area
        var dX = (int) Math.Round(GameState.FogOffsetX * 2.5d - 50d);
        var dY = (int) Math.Round(GameState.FogOffsetY * 3.5d - 50d);
        var dW = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Fogs, fogNum.ToString())).Width + 200;
        var dH = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Fogs, fogNum.ToString())).Height + 200;

        var argpath = System.IO.Path.Combine(DataPath.Fogs, fogNum.ToString());
            ...
            for (i = (int) MapLayer.Ground; i <= (int) MapLayer.CoverAnimation; i++)
            {
                    ...
                    Data.MyMap.Tile[x, y].Layer[layerIndex].Tileset <= GameState.NumTileSets)
                {
                    ...
                    if (Data.Autotile[x, y].Layer[layerIndex].RenderState == GameState.RenderStateNormal)
                    {
                        ...
                        // Render the tile
                        var argpath = System.IO.Path.Combine(DataPath.Tilesets, Data.MyMap.Tile[x, y].Layer[layerIndex].Tileset.ToString());
    ...
    public static void DrawAutoTile(int layerNum, int dX, int dY, int quarterNum, int x, int y, int forceFrame = 0, bool strict = true)
    {
        ...
        if (Data.MyMap.Tile[x, y].Layer is null)
            return;
        var argpath = System.IO.Path.Combine(DataPath.Tilesets, Data.MyMap.Tile[x, y].Layer[layerNum].Tileset.ToString());
    ...
    public static void DrawPanorama(int index)
    {
        ...
        var argpath = System.IO.Path.Combine(DataPath.Panoramas, index.ToString());
    ...
    public static void DrawParallax(int index)
    {
        ...
        var argpath = System.IO.Path.Combine(DataPath.Parallax, index.ToString());
    ...
    public static void DrawPicture(int index = 0, int type = 0)
    {
        ...
        var argpath = System.IO.Path.Combine(DataPath.Pictures, index.ToString());
    ...
    public static void ClearMap()
    {
        ...
        for (var i = 0; i < Constant.MaxMapNpcs; i++)
        {
            Data.MyMap.Npc[i] = -1;
        }
        ...
        for (var i = 0; i < GameState.MaxTileHistory; i++)
        {
        ...
        for (var x = 0; x < Data.MyMap.MaxX; x++)
        {
            for (var y = 0; y < Data.MyMap.MaxY; y++)
            {
    ...
    {
        for (var i = 0; i < Constant.MaxMapItems; i++)
            ClearMapItem(i);
    }
    ...
    public static void ClearMapNpc(int index)
    {
        ...
        for (var i = 0; i < Enum.GetValues(typeof(Vital)).Length; i++)
        {
            withBlock.Vital[i] = 0;
        }
    ...
    public static void SendMap()
    {
        ...
        for (x = 0; x < Data.MyMap.MaxX; x++)
        {
            for (y = 0; y < Data.MyMap.MaxY; y++)
            {
                ...
                var layerCount = Enum.GetValues<MapLayer>().Length;
                for (i = 0; i < layerCount; i++)
                {
