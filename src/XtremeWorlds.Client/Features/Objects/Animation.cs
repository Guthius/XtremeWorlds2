...
public class Animation
{
    ...
    {
        var sprite = Data.Animation[AnimInstance[index].Animation].Sprite[layer];
        ...
        var totalWidth = gfxInfo.Width;
        var totalHeight = gfxInfo.Height;
        var columns = Data.Animation[AnimInstance[index].Animation].Frames[layer];
        ...
        {
            var lockindex = AnimInstance[index].LockIndex;
        ...
        var argpath = System.IO.Path.Combine(DataPath.Animations, sprite.ToString());
    ...
    public static void CheckAnimInstance(int index)
    {
        ...
        // Get dimensions and column count from controls and graphic info
        var totalWidth = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Animations, Data.Animation[AnimInstance[index].Animation].Sprite[layer].ToString())).Width;
        var totalHeight = GameClient.GetGfxInfo(System.IO.Path.Combine(DataPath.Animations, Data.Animation[AnimInstance[index].Animation].Sprite[layer].ToString())).Height;
        var columns = Data.Animation[AnimInstance[index].Animation].Frames[layer];

        // Calculate frame dimensions
        var frameWidth = (int)Math.Round(totalWidth / (double)columns);
        var frameHeight = frameWidth;
        var rows = default(int);
        if (frameHeight > 0)
        {
    ...
    public static int PlayAnimation(int sprite, int layer, int data, byte x, byte y)
    {
        ...
        var totalWidth = gfxInfo.Width;
        var totalHeight = gfxInfo.Height;
        var columns = Data.Animation[data].Frames[layer];
        var frameWidth = 0;
        var rows = 0;
    ...
    public static void ClearAnimInstance(int index)
    {
        ...
        for (var i = 0; i < AnimInstance[index].Used.Length; i++)
            AnimInstance[index].Used[i] = false;

        for (var i = 0; i < AnimInstance[index].Timer.Length; i++)
            AnimInstance[index].Timer[i] = 0;

        for (var i = 0; i < AnimInstance[index].FrameIndex.Length; i++)
