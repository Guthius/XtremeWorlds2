...
using Autotile = XtremeWorlds.Client.Features.Objects.Autotile;
using Bank = XtremeWorlds.Client.Features.Systems.Bank;
using Map = XtremeWorlds.Client.Features.Objects.Map;
using Party = XtremeWorlds.Client.Features.Systems.Party;
using Projectile = XtremeWorlds.Client.Features.Objects.Projectile;
...
public class GameLogic
{
    ...
    {
        var convertCurrency = string.Empty;
    ...
    public static void HandlePressEnter()
    {
        ...
        if (Strings.Left(chatText, 1) == "!")
        {
            ...
            {
                TextRenderer.AddText("Usage : ![player] [message]", (int) ColorName.Yellow);
            }
        ...
        if (Strings.Left(chatText, 1) == "/")
        {
            ...
            switch (command[0] ?? "")
            {
                case "/emote":
                {
                    ...
                    {
                        TextRenderer.AddText("Usage : /emote [1-11]", (int) ColorName.Yellow);
                        goto Continue1;
                    }
                ...
                case "/help":
                {
                    TextRenderer.AddText("Social Commands : ", (int) ColorName.Yellow);
                    TextRenderer.AddText("'[message] = Global Message", (int) ColorName.Yellow);
                    TextRenderer.AddText("-[message] = Party Message", (int) ColorName.Yellow);
                    TextRenderer.AddText("![player] [message] = Player Message", (int) ColorName.Yellow);
                    TextRenderer.AddText("@[message] = Admin Message", (int) ColorName.Yellow);
                    TextRenderer.AddText("""
                                         Available Commands: /help, /info, 
                                         /fps, /lps, /stats, /trade, 
                                         /party, /join, /leave
                                         """, (int) ColorName.Yellow);
                    break;
                }
                ...
                case "/info":
                {
                    ...
                    if (Information.UBound(command) < 1 || Information.IsNumeric(command[1]))
                    {
                        TextRenderer.AddText("Usage : /info [player]", (int) ColorName.Yellow);
                        goto Continue1;
                    }
                ...
                case "/admin":
                {
                    ...
                    TextRenderer.AddText("""
                                         ...
                                         """, (int) ColorName.Yellow);
                    TextRenderer.AddText( "''msghere = Global Admin Message", (int) ColorName.Yellow);
                    TextRenderer.AddText("= msghere = Private Admin Message", (int) ColorName.Yellow);
                    break;
                }
    ...
    {
        // Loop through and see if that player/Npc already has a chat bubble
        for (var i = 0; i <= GameState.ChatBubbleindex; i++)
        {
    ...
    public static void ShowItemDesc(int x, int y, int itemNum)
    {
        ...
        // set variables
        {
            var winDescription = Gui.Windows[Gui.GetWindowIndex("winDescription")];
            // name
            // If Not soulBound Then
            theName = Data.Item[itemNum].Name;
            // Else
            // theName = "(SB) " & Item(itemNum).Name)
            // End If
            winDescription.GetChild("lblName").Text = theName;
            switch (Data.Item[itemNum].Rarity)
            {
                case 0: // white
                {
            ...
            winDescription.GetChild<Label>("lblName").Color = color;

            // class req
            if (Data.Item[itemNum].JobReq > 0)
            {
                jobName = Data.Job[Data.Item[itemNum].JobReq].Name;
                // do we match it?
                if (GetPlayerJob(GameState.MyIndex) == Data.Item[itemNum].JobReq)
                {
                    color = Microsoft.Xna.Framework.Color.Green;
                }
                else
            ...
            winDescription.GetChild<Label>("lblJob").Text = jobName;
            winDescription.GetChild<Label>("lblJob").Color = color;
            
            // level
            if (Data.Item[itemNum].LevelReq > 0)
            {
                levelTxt = "Level " + Data.Item[itemNum].LevelReq;
                // do we match it?
                if (GetPlayerLevel(GameState.MyIndex) >= Data.Item[itemNum].LevelReq)
                {
                    color = Microsoft.Xna.Framework.Color.Green;
                }
                else
            ...
            winDescription.GetChild<Label>("lblLevel").Text = levelTxt;
            winDescription.GetChild<Label>("lblLevel").Color = color;
        }
        ...
        switch (Data.Item[itemNum].Type)
        {
            ...
            {
                switch ((Equipment) Data.Item[itemNum].SubType)
                {
        ...
        switch (Data.Item[itemNum].Type)
        {
            ...
            {
                // binding
                if (Data.Item[itemNum].BindType == 1)
                {
                    AddDescInfo("Bind on Pickup", Microsoft.Xna.Framework.Color.White);
                }
                else if (Data.Item[itemNum].BindType == 2)
                {
                    AddDescInfo("Bind on Equip", Microsoft.Xna.Framework.Color.White);
                }

                AddDescInfo("Value: " + Data.Item[itemNum].Price + " g", Microsoft.Xna.Framework.Color.Yellow);
                break;
            }
            case (byte) ItemCategory.Equipment:
            {
                // Damage/defense
                if (Data.Item[itemNum].SubType == (byte) Equipment.Weapon)
                {
                    AddDescInfo("Damage: " + Data.Item[itemNum].Data2, Microsoft.Xna.Framework.Color.White);
                    AddDescInfo("Speed: " + Data.Item[itemNum].Speed / 1000d + "s", Microsoft.Xna.Framework.Color.White);
                }
                else if (Data.Item[itemNum].Data2 > 0)
                {
                    AddDescInfo("Defense: " + Data.Item[itemNum].Data2, Microsoft.Xna.Framework.Color.White);
                }
                ...
                // stat bonuses
                if (Data.Item[itemNum].AddStat[(int) Stat.Strength] > 0)
                {
                    AddDescInfo("+" + Data.Item[itemNum].AddStat[(int) Stat.Strength] + " Str", Microsoft.Xna.Framework.Color.White);
                }

                if (Data.Item[itemNum].AddStat[(int) Stat.Luck] > 0)
                {
                    AddDescInfo("+" + Data.Item[itemNum].AddStat[(int) Stat.Luck] + " End", Microsoft.Xna.Framework.Color.White);
                }

                if (Data.Item[itemNum].AddStat[(int) Stat.Spirit] > 0)
                {
                    AddDescInfo("+" + Data.Item[itemNum].AddStat[(int) Stat.Spirit] + " Spi", Microsoft.Xna.Framework.Color.White);
                }

                if (Data.Item[itemNum].AddStat[(int) Stat.Luck] > 0)
                {
                    AddDescInfo("+" + Data.Item[itemNum].AddStat[(int) Stat.Luck] + " Luc", Microsoft.Xna.Framework.Color.White);
                }

                if (Data.Item[itemNum].AddStat[(int) Stat.Intelligence] > 0)
                {
                    AddDescInfo("+" + Data.Item[itemNum].AddStat[(int) Stat.Intelligence] + " Int", Microsoft.Xna.Framework.Color.White);
                }
    ...
    public static void ShowSkillDesc(int x, int y, int skillNum, long skillSlot)
    {
        ...
        // set variables
        {
            var winDescription = Gui.Windows[Gui.GetWindowIndex("winDescription")];
            // set name
            winDescription.GetChild<Label>("lblName").Text = Data.Skill[skillNum].Name;
            winDescription.GetChild<Label>("lblName").Color = Microsoft.Xna.Framework.Color.White;
            ...
            if (skillSlot >= 0L)
            {
                ...
                tmpWidth = 66;
                // End If
               // TODO: winDescription.Controls[Gui.GetControlIndex("winDescription", "picBar")].Value = tmpWidth;
                // does it rank up?
                ...
                // End If
                // show controls
                winDescription.Controls[Gui.GetControlIndex("winDescription", "lblJob")].Visible = true;
                winDescription.Controls[Gui.GetControlIndex("winDescription", "picBar")].Visible = true;
                // set vals
                winDescription.GetChild<Label>("lblJob").Text = sUse;
                winDescription.GetChild<Label>("lblJob").Color = Microsoft.Xna.Framework.Color.White;
            }
            else
            {
                // hide some controls
                winDescription.Controls[Gui.GetControlIndex("winDescription", "lblJob")].Visible = false;
                winDescription.Controls[Gui.GetControlIndex("winDescription", "picBar")].Visible = false;
            }
        }

        switch (Data.Skill[skillNum].Type)
        {
        ...
        switch (Data.Skill[skillNum].Type)
        {
            ...
            {
                // damage
                AddDescInfo("Vital: " + Data.Skill[skillNum].Vital, Microsoft.Xna.Framework.Color.White);

                // mp cost
                AddDescInfo("Cost: " + Data.Skill[skillNum].MpCost + " SP", Microsoft.Xna.Framework.Color.White);

                // cast time
                AddDescInfo("Cast Time: " + Data.Skill[skillNum].CastTime + "s", Microsoft.Xna.Framework.Color.White);

                // cd time
                AddDescInfo("Cooldown: " + Data.Skill[skillNum].CdTime + "s", Microsoft.Xna.Framework.Color.White);

                // aoe
                if (Data.Skill[skillNum].AoE > 0)
                {
                    AddDescInfo("AoE: " + Data.Skill[skillNum].AoE, Microsoft.Xna.Framework.Color.White);
                }

                // stun
                if (Data.Skill[skillNum].StunDuration > 0)
                {
                    AddDescInfo("Stun: " + Data.Skill[skillNum].StunDuration + "s", Microsoft.Xna.Framework.Color.White);
                }

                // dot
                if (Data.Skill[skillNum].Duration > 0 & Data.Skill[skillNum].Interval > 0)
                {
                    AddDescInfo("DoT: " + Data.Skill[skillNum].Duration / (double) Data.Skill[skillNum].Interval + " tick", Microsoft.Xna.Framework.Color.White);
                }
    ...
    public static void OpenShop(long shopNum)
    {
        ...
        // TODO: Gui.Windows[Gui.GetWindowIndex("winShop")].Controls[Gui.GetControlIndex("winShop", "CheckboxSelling")].Value = 0;
        // TODO: Gui.Windows[Gui.GetWindowIndex("winShop")].Controls[Gui.GetControlIndex("winShop", "CheckboxBuying")].Value = 0;
    ...
    public static void UpdatePartyBars()
    {
        ...
        // make sure we're in a party
        {
            ...
            for (i = 0L; i <= 3L; i++)
            {
                ...
                {
                    // TODO: pIndex = withBlock.Controls[Gui.GetControlIndex("winParty", "picChar" + i)].Value;
    ...
    public static List<Microsoft.Xna.Framework.Vector2> GetCellsInSquare(int xCenter, int yCenter, int distance)
    {
        ...
        var xMax = Math.Min(Data.MyMap.MaxX, xCenter + distance);
        var yMin = Math.Max(0, yCenter - distance);
        var yMax = Math.Min(Data.MyMap.MaxY, yCenter + distance);
    ...
    private static List<Microsoft.Xna.Framework.Vector2> Line(int x, int y, int xDestination, int yDestination)
    {
        ...
        var dx = Math.Abs(xDestination - x);
        var dy = Math.Abs(yDestination - y);
        var sx = x < xDestination ? 1 : -1;
        var sy = y < yDestination ? 1 : -1;
        var err = dx - dy;
    ...
    public static List<Microsoft.Xna.Framework.Vector2> AppendFov(int xOrigin, int yOrigin, int radius, bool lightWalls)
    {
        ...
        foreach (var borderCell in GetBorderCellsInSquare(xOrigin, yOrigin, radius))
        {
            // Trace a line from the origin to the border cell
            foreach (var cell in Line(xOrigin, yOrigin, (int) Math.Round(borderCell.X), (int) Math.Round(borderCell.Y)))
            {
        ...
        if (lightWalls)
        {
            foreach (var cell in GetCellsInSquare(xOrigin, yOrigin, radius))
            {
    ...
    public static void UpdateCamera()
    {
        ...
        var tileWidth = (long) Math.Round(GameState.ResolutionWidth / 32d);
        var tileHeight = (long) Math.Round(GameState.ResolutionHeight / 32d);

        var StartX = Math.Max(0, Math.Min((long) Math.Floor(GameState.Camera.Left), Data.MyMap.MaxX - 1) / 32);
        var StartY = Math.Max(0, Math.Min((long) Math.Floor(GameState.Camera.Top), Data.MyMap.MaxY - 1) / 32);
